...
export default class GoogleMapsComponent {
	...
	onConfirm: Function;
	pacInput: HTMLInputElement | null;
	containerPac: HTMLElement | null;
	containerDetails: HTMLElement | null;
	mapsInputForm: HTMLElement | null;
	mapsTitle: HTMLElement | null;
	mapsSubtitle: HTMLElement | null;
	map: any | null;
	marker: any | null;
	myLatLng: any | null;
	...
	render(): void {
		...
		const componentHTML = ` <div class="container-pac" id="container-pac"> <div class="bonus-container-header"> <img class="bonus-input-icon-back" style="cursor: pointer;" id="container-back" src="back-map.svg" alt="" /> <div class="bonus-container-input-place"> <input id="pac-input" class="input-place" type="text" placeholder="Buscar lugares" /> <img id="image-clear" style="cursor: pointer;" src="clear-map.svg" alt="" /> </div> </div> <p class="bonus-map-tooltip"> <a class="find-address">¿No encuentras tu dirección?</a> </p> <div id="maps-input-form" class="maps-input-form"> <!-- Contenedor para el pac-container --> </div> </div> <div class="container-details display-off" id="container-details"> <div style="display: flex;align-items: center;"> <img style="cursor: pointer;margin-right: 10px;" id="maps-register-back" class="bonus-input-icon-back" src="back-map.svg" /> <h4 class="bonus-map-title bonus-text-sub-title bonus-text-sub-title-bold">Verifica la ubicación</h4> </div> <div id="map" class="map"></div> <div class="container-button-text"> <div style="display: flex;align-items: center;justify-content: space-around;margin-top: 20px;margin-bottom: 20px;"> <div> <p id="maps-title" class="bonus-map-address-title"></p> <p id="maps-sub-title" class="bonus-map-address-sub-title"></p> </div> <div id="maps-find" class="bonus-maps-find"> <img src="spot.svg" alt="" /> <p style="margin-left: 5px;">Encuéntrame</p> </div> </div> <div style="display: grid; justify-content: center;"> <button id="bonus-button" class="bonus-button bonus-button-yellow"> CONFIRMAR UBICACIÓN </button> <p class="bonus-map-tooltip"> <a class="find-address">¿No encuentras tu dirección?</a> </p> </div> </div> </div>`;
	...
	addEventListeners(): void {
		...
		this.pacInput?.addEventListener('keyup', (e: any) => {
		...
		this.containerBack?.addEventListener('click', () => {
			this.onBack();
		});

		// evento al retroceder a seleccionar lugar
		this.mapsRegisterBack?.addEventListener('click', () => {
			this.onBackSelectPlace();
		});

		//evento eliminar pac-items
		this.imageClear?.addEventListener('click', () => {
			this.onClearInputPlace();
		});

		// evento para encontrar mi localización
		this.mapsFind?.addEventListener('click', () => {
			this.onFindMe();
		});

		// evento al seleccionar que no encuentra su dirección en google maps
		this.findAddress?.forEach((element: any) => {
	...
	movePacContainer(): void {
		const pacContainer: any = document.querySelector('.pac-container');
		if (pacContainer) {
	...
	initAutocomplete(): void {
		const autocomplete = new google.maps.places.Autocomplete(
			this.pacInput as HTMLInputElement
		);
        ...
        // limpiar input y borrar la lista de auto completado
				this.onClearInputPlace();
        // mostrar mapa
				(document.getElementById('map') as HTMLElement).style.opacity = '1';
        ...
        // texto inferior de detalles del mapa
				...
				this.mapsSubtitle!.innerText = `${this.formatted_address![2]} - ${
					this.formatted_address![1]
				}`;
			}
			this.onNextStep();
		});
  ...
  // geocoding de latitud y longitud para obtener un json del lugar
	geocodingOfCoords(LatLng: any): void {
